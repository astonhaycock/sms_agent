"""
Build the camping and outdoors advice vector store from PDFs in camping_advice/pdf/.
Uses Ollama nomic-embed-text:latest and persists a FAISS index to vector-database/.

Run from langgraph dir: uv run python -m agent_router.camping_advice.build_vector_store
Or: cd agent_router/camping_advice && python build_vector_store.py (with PYTHONPATH set)
"""
...
PDF_DIR = CAMPING_ADVICE_DIR / "pdf"
...
def load_pdfs() -> list:
    """Load all PDFs from camping_advice/pdf/ into LangChain documents."""
    docs = []
